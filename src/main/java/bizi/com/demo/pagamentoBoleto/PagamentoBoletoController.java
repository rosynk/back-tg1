...
public class PagamentoBoletoController {
    ...
    public ResponseEntity<?> realizarPagamento(@Valid @RequestBody PagamentoBoletoDto dto) {
        try {
            PagamentoBoletoDto response = pagamentoBoletoService.realizarPagamento(dto);
    ...
    public ResponseEntity<PagamentoBoletoModel> buscarPorId(
            @Parameter(description = "ID do pagamento") @PathVariable Long id) {
        try {
            return ResponseEntity.ok(pagamentoBoletoService.buscarPorId(id));
    ...
    public ResponseEntity<List<PagamentoBoletoModel>> buscarPorConta(
